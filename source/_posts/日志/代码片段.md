...
Long maxId = 0L;
while (true){
    DataCompareResultExample dataCompareResultExample = new DataCompareResultExample();
    dataCompareResultExample.clear();
    dataCompareResultExample.setOrderByClause("id");
    dataCompareResultExample
            .createCriteria()
            .andIdGreaterThan(maxId)
            .andDeleteFlagEqualTo(DeleteFlag.ENABLE.getIndex());
    PageHelper.offsetPage(0,1000,false);
    List<DataCompareResult> dataCompareResults = datacompareResultService.selectByExample(dataCompareResultExample);
    if (CollectionUtils.isEmpty(dataCompareResults)) {
        break;
    }
    maxId = dataCompareResults.get(dataCompareResults.size()-1).getId();
}
```

# 按天处理数据
```java
    @GetMapping(value = "/refresh/fundFlow")
    public BaseResult refreshFundFLow(@RequestParam(required = false) String dateBegin, @RequestParam(required = false) String dateEnd) throws Exception {
        Date begin = DateUtils.parseDate(dateBegin, "yyyyMMdd");
        Date end = DateUtils.parseDate(dateEnd, "yyyyMMdd");

        if (begin.after(end)) {
            return new BaseResult(BaseMessage.FAILED.getCode(), "开始时间不可大于结束时间", "");
        }
        while (end.after(begin) || begin.equals(end)) {
            liquidateFundFlowService.refreshFundFLow(begin);
...
```java
    @Override
    public void refreshFundFLow(Date tranDate) {
        log.info("刷新资金流水业务单据信息:{}",DateFormatUtils.format(tranDate,"yyyyMMdd"));
        ...
        while (true){
            PageHelper.offsetPage(0, 1000, false);
            LiquidateFundFlowExample example = new LiquidateFundFlowExample();
            LiquidateFundFlowExample.Criteria criteria = example.createCriteria()
                    .andDeleteFlagEqualTo(DeleteFlag.ENABLE.getIndex())
                    .andTranDateEqualTo(tranDate)
                    .andCompareStatusEqualTo(CompareStatusEnum.SUCCESS.getIndex());
            example.setOrderByClause("id");
            if (null != maxId){
                criteria.andIdGreaterThan(maxId);
            }
            List<LiquidateFundFlow> liquidateFundFlows = getMapper().selectByExample(example);
            if (CollectionUtils.isEmpty(liquidateFundFlows)){
                break;
            }
            maxId = liquidateFundFlows.get(liquidateFundFlows.size() - 1).getId();
```
